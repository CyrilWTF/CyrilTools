...
class RulerView(context: Context, attrs: AttributeSet?, defStyleAttr: Int, defStyleRes: Int) :
    ...
    private val mPadding = 16 * context.resources.displayMetrics.density
        ...
        canvas?.run {
            ...
            while (now < height - mPadding) {
                when {
                    ...
                    scale % 5 == 0 -> drawLine(width - 72f, now, width, now, mPaint)
                    else -> drawLine(width - 48f, now, width, now, mPaint)
                }
                now += mScaleGapPixel
                ++scale
            }
